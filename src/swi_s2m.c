...
//             LOG_I(common, "<-- CMD(%u) len(%u)", channel, req->cmd.type, req->cmd.len);
            size_t bWrite = s2m_info->hw_write(s2m_info->wr_hndl, (const uint32_t*)req,
...
//                 LOG_W(common, "channel(%u) blocked writer", channel);
                ...
                uxBits = xEventGroupWaitBits(s2m_info->evt_grp, SWI_S2M_UNBLOCK_WRITER, pdTRUE,
...
int32_t swi_s2m_init(swi_s2m_info_t* s2m_info, write_op hw_write, void* wr_hndl, unsigned int idx)
{
    ...
    EventBits_t uxBits = xEventGroupClearBits(s2m_info->evt_grp,
                                              SWI_S2M_BLOCKED_WRITER | SWI_S2M_UNBLOCK_WRITER);
