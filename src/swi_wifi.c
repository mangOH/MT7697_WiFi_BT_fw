...
static int32_t swi_wifi_proc_set_pmk_req(swi_m2s_info_t* m2s_info)
{
    ...
    if (memcmp(pmk, empty_pmk, WIFI_LENGTH_PMK)) {
        ...
        if (port == WIFI_PORT_AP) {
            ret = wifi_config_set_security_mode(port, WIFI_AUTH_MODE_WPA2_PSK ,
        ...
        ret = wifi_config_set_security_mode(port, WIFI_AUTH_MODE_OPEN,
...
static int32_t swi_wifi_proc_get_radio_state_req(swi_m2s_info_t* m2s_info)
{
    ...
    mt7697_get_radio_state_rsp_t* rsp = (mt7697_get_radio_state_rsp_t*)swi_mem_pool_alloc_msg(
        &wifi_info.s2m_info->msg_pool_info, SWI_MEM_POOL_MSG_HI_PRIORITY, wifi_info.s2m_info->sendQ,
    ...
    if (m2s_info->cmd_hdr.len != sizeof(mt7697_get_radio_state_req_t)) {
        LOG_W(common, "invalid get radio state req len(%u != %u)", m2s_info->cmd_hdr.len,
              sizeof(mt7697_get_radio_state_req_t));
        ret = -1;
        goto cleanup;
    }
...
static int32_t swi_wifi_proc_set_radio_state_req(swi_m2s_info_t* m2s_info)
{
    ...
    mt7697_set_radio_state_rsp_t* rsp = (mt7697_set_radio_state_rsp_t*)swi_mem_pool_alloc_msg(
        &wifi_info.s2m_info->msg_pool_info, SWI_MEM_POOL_MSG_HI_PRIORITY, wifi_info.s2m_info->sendQ,
    ...
    if (m2s_info->cmd_hdr.len != sizeof(mt7697_set_radio_state_req_t)) {
        LOG_W(common, "invalid radio set state req len(%u != %u)", m2s_info->cmd_hdr.len,
              sizeof(mt7697_set_radio_state_req_t));
        ret = -1;
        goto cleanup;
    }

    size_t words_read = m2s_info->hw_read(m2s_info->rd_hndl, (uint32_t*)&state,
                                          LEN_TO_WORD(sizeof(uint32_t)));
    if (words_read != LEN_TO_WORD(sizeof(uint32_t))) {
        LOG_W(common, "hw_read() failed(%d != %d)", words_read, LEN_TO_WORD(sizeof(uint32_t)));
        ret = -1;
