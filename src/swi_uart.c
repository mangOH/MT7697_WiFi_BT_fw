...
//            LOG_I(common, "S2M unblock Tx");
            uxBits = xEventGroupSetBitsFromISR(uart_info->s2m.evt_grp, SWI_S2M_UNBLOCK_WRITER,
...
//            LOG_I(common, "M2S unblock Rx");
            uxBits = xEventGroupSetBitsFromISR(uart_info->m2s.evt_grp, SWI_M2S_UNBLOCK_READER,
                                               &xHigherPriorityTaskWoken);
            if (xHigherPriorityTaskWoken)
                portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
        }
...
static int32_t swi_uart_proc_shutdown_req(swi_m2s_info_t* m2s_info)
{
    ...
    mt7697_uart_shutdown_rsp_t* rsp = (mt7697_uart_shutdown_rsp_t*)swi_mem_pool_alloc_msg(
        &uart_info->s2m.msg_pool_info, SWI_MEM_POOL_MSG_HI_PRIORITY, uart_info->s2m.sendQ,
...
//        LOG_W(common, "S2M blocked Tx");
        ...
        uxBits = xEventGroupWaitBits(uart_info->s2m.evt_grp, SWI_S2M_UNBLOCK_WRITER, pdTRUE, pdTRUE,
                                     portMAX_DELAY);
        configASSERT(uxBits & SWI_S2M_BLOCKED_WRITER);
...
//    LOG_W(common, "M2S blocked Rx");
        ...
        uxBits = xEventGroupWaitBits(uart_info->m2s.evt_grp, SWI_M2S_UNBLOCK_READER, pdTRUE, pdTRUE,
                                     portMAX_DELAY);
        configASSERT(uxBits & SWI_M2S_BLOCKED_READER);
...
int32_t swi_uart_init(swi_uart_info_t* uart_info)
{
    ...
    ret = swi_mem_pool_init(&uart_info->s2m.msg_pool_info.lo, SWI_MSG_POOL_LEN,
                            MT7697_IEEE80211_FRAME_LEN);
    if (ret < 0) {
        LOG_W(common, "swi_mem_pool_init() failed(%d)", ret);
        goto cleanup;
    }

    ret = swi_mem_pool_init(&uart_info->s2m.msg_pool_info.hi, SWI_MSG_POOL_HI_LEN,
                            MT7697_IEEE80211_FRAME_LEN);
    if (ret < 0) {
        LOG_W(common, "swi_mem_pool_init() failed(%d)", ret);
        goto cleanup;
