...
{
    swi_mem_pool_t* msg_pool = ((ptr >= mem_pool_info->lo.start) && (ptr < mem_pool_info->lo.end)) ?
        &mem_pool_info->lo : &mem_pool_info->hi;
    int ret = 0;
...
uint8_t* swi_mem_pool_alloc_msg(swi_mem_pool_info_t* mem_pool_info, uint16_t priority,
                                QueueHandle_t sendQ, uint16_t len)
{
    ...
    if (msg_pool->start != NULL) {
        if (!uxQueueSpacesAvailable(sendQ)) {
            ...
            goto cleanup;
                 ...
                 (uxQueueSpacesAvailable(sendQ) < SWI_MEM_POOL_SENDQ_HIGH_WATER_MARK)) {
...
//         LOG_I(common, "alloc(%u: %p -> %p) free(%p) start/end(%p/%p)", len, msg_pool->alloc_ptr,
//               msg_pool->alloc_ptr + len, msg_pool->free_ptr, msg_pool->start, msg_pool->end);
        if ((msg_pool->alloc_ptr >= msg_pool->free_ptr) ||
            ((msg_pool->alloc_ptr + len) < msg_pool->free_ptr)) {
...
//         LOG_I(common, "init msg pool/len(%u)", len);
    ...
    msg_pool->alloc_ptr = msg_pool->start;
    msg_pool->free_ptr = msg_pool->end;
//     LOG_I(common, "alloc/free(%p/%p) start/end(%p/%p)", msg_pool->alloc_ptr, msg_pool->free_ptr,
//           msg_pool->start, msg_pool->end);
